...
- `target` contains the column name that has the values to be plotted in the bars.
- `grouping` contains the column name that has the categorical groupings. In most line charts, this parameter refers to the year or time variable.
...
Sometimes, you want to track the evolution of some variables over time but you want to focus only in two points in time. For this specific cases a slope chart is a better option in aesthetic terms than a line chart. The `wjp_slope()` function can help you in that. I will exemplify the use of this chart when we want to disaggregate the evolution of trust in Institution A between 2017 and 2022 in all three countries. We start by preparing the data for our new design:
...
Once we have the data ready for our slopes, we proceed to use the `wjp_slope()` function. All the required parameters for this function have been covered in previous examples:

- `target` contains the column name that has the values to be plotted in the bars.
- `grouping` contains the column name that has the categorical groupings. In most slope charts, this parameter refers to the year or time variable.
- `colors` contains the column name that will distinguish the different colors in the slope chart.
- `cvec` contains a named vector linking values to specific color codes.
- `labels` contains the column name that has the value labels to be displayed, if any.
- `ngroups` refers to the number of lines to be plotted. If only one, please pass "1" if more than 1 line is to be plotted, then pass the column with the groups as a vector.
- `repel` activates the ggrepel feature to avoid label overlapping.
...
When we need to plot multiple variables and show the results for multiple agents or regions, a dot chart comes very handy. The `wjp_dots()` functions helps you to chart the data as quickly as possible. Let's try to plot the percentage of people who had a lot or some trust in institutions A, B, C, and D in all three countries: Atlantis, Narnia, and Neverland. We begin by preparing our data for a minimal example as follows:
...
Once we have the data ready for our slopes, we proceed to use the `wjp_dots()` function. All the required parameters for this function have been covered in previous examples:
